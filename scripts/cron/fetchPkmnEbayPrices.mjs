...
/**
 * Usage:
 *   node scripts/cron/fetchPkmnEbayPrices.mjs --all --limit=0 --batch=800 --concurrency=4
 *   node scripts/cron/fetchPkmnEbayPrices.mjs --only-missing-ebay --days=3 --base=https://legendary-collectibles.com
 *
 * Requires in env (same as your MTG job because the API route uses them):
 *   EBAY_CLIENT_ID
 *   EBAY_CLIENT_SECRET
 *   CRON_SECRET
 *
 * Optional:
 *   EBAY_MARKETPLACE, EBAY_SCOPE, EBAY_PRICE_BASE_URL
 */
...
const FLAG_ONLY_MISSING_PRIM = flags.has('only-missing-primary'); // kept for API parity (no-op by default)
const FLAG_ONLY_MISSING_EBAY = flags.has('only-missing-ebay');    // prefers tcg_card_prices_ebay.updated_at if table exists
...
const PRICE_ENDPOINT = (id) =>
  `${BASE_URL}/api/ebay/price/${encodeURIComponent(id)}?persist=1&game=pokemon`;
...
let HAS_TCG_EBAY_TABLE = false;
(async () => {
  HAS_TCG_EBAY_TABLE = await tableExists('tcg_card_prices_ebay');
})().catch(() => { /* ignore */ });

async function getIdsPage({ after, limit }) {
  // Keyset pagination on c.id (text PK like 'base1-1')
  const whereParts = [];
  if (after) whereParts.push(sql`c.id > ${after}`);
  else whereParts.push(sql`TRUE`);

  // Optional: only-missing-primary (Pokémon primary = external feeds; keep as a no-op unless you add joins)
  if (FLAG_ONLY_MISSING_PRIM) {
    // If you want, join tcgplayer/cardmarket tables and check nulls. For now, allow all.
    whereParts.push(sql`TRUE`);
  }

  if (FLAG_ONLY_MISSING_EBAY && HAS_TCG_EBAY_TABLE) {
    // Use our per-game ebay table freshness if available
    whereParts.push(sql`
      NOT EXISTS (
        SELECT 1
        FROM public.tcg_card_prices_ebay e
        WHERE e.cardid = c.id
...
async function fetchOne(id) {
  ...
  if (FLAG_VERBOSE) console.log('[ebay/pkmn] GET', url);
  ...
  if (!res.ok) {
    ...
    if (!isJson(res)) {
      console.warn(`[ebay/pkmn] Non-JSON (status ${res.status}) from ${url}`);
      console.warn('            First 200 chars:', body.slice(0, 200).replace(/\s+/g, ' '));
    } else {
      console.warn(`[ebay/pkmn] ${res.status} JSON error from ${url}:`, body.slice(0, 500));
    }
...
const CURSOR_FILE = `${STATE_DIR}/ebay-pokemon.cursor`;
...
// -------- main loop --------
console.log('Pokémon eBay harvester starting…');
console.log({
  base: BASE_URL,
  all: FLAG_ALL,
