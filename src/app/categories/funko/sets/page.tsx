...
export default async function FunkoSetsIndex({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-sm text-white/80">Showing {from}-{to} of {total} series{q ? " (filtered)" : ""}</div>
        <div className="flex flex-wrap gap-3">
          ...
          <form action={baseHref} method="get" className="flex items-center gap-2">
            ...
            <input type="hidden" name="page" value="1" />
            <input name="q" defaultValue={q ?? ""} placeholder="Search series (name/id)…" className="w-[240px] md:w-[320px] rounded-lg border border-white/20 bg-white/10 px-3 py-1.5 text-sm text-white placeholder:text-white/60 outline-none focus:ring-2 focus:ring-white/50" />
      ...
      {rows.length === 0 ? (
        <div className="rounded-xl border border-white/15 bg-white/5 p-6 text-white/90 backdrop-blur-sm">No series yet — data coming soon.</div>
      ) : (
