...
async function getPriceSummary(_opts:{ setId:string }):Promise<{ tcgplayerRows:Array<{label:string;value:string|null}>; cardmarketRows:Array<{label:string;value:string|null}>;}>{
  return {
    tcgplayerRows: [{label:"Common",value:null},{label:"Exclusive",value:null}],
    cardmarketRows: [{label:"Avg price",value:null},{label:"Trend price",value:null}],
  };
}

export default async function FunkoSetPricesPage({ params, searchParams }:{ params: Promise<{id:string}>; searchParams: Promise<SearchParams>; }) {
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={baseSetHref} className="text-sky-300 hover:underline">← Back to series</Link>
        </div>
      </div>
