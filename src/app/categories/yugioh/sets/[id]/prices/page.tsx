...
/* ---------- Data stubs (drop SQL later) ---------- */
async function getSet(setId: string): Promise<SetRow | null> {
  // When ready, fetch real set header (logo/symbol/name)
  return { id: setId, name: setId.replace(/-/g, " "), logo_url: null, symbol_url: null };
}
async function getPriceSummary(_opts: {
  setId: string;
}): Promise<{ tcgplayerRows: Array<{ label: string; value: string | null }>; cardmarketRows: Array<{ label: string; value: string | null }> }> {
  // Later: aggregate rows per set (medians/averages or representative prices)
  return {
    tcgplayerRows: [
      { label: "Normal", value: null },
      { label: "Holofoil", value: null },
      { label: "1st Edition", value: null },
    ],
    cardmarketRows: [
      { label: "Average sell price", value: null },
      { label: "Trend price", value: null },
      { label: "Low (EX+)", value: null },
    ],
  };
}
...
export default async function YugiohSetPricesPage({
  ...
  // Try to use shared pricing helpers if present; fall back gracefully.
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-xl border border-white/15 bg-white/5 p-5 text-white/90 backdrop-blur-sm">
        ...
        {!tcgplayerRows.some(r => r.value) && (
          <div className="mt-3 rounded-md border border-white/10 bg-white/5 p-3 text-sm text-white/80">
            No TCGplayer price rows yet — wiring will mirror the Pokémon page.
          </div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-white/15 bg-white/5 p-5 text-white/90 backdrop-blur-sm">
        ...
        {!cardmarketRows.some(r => r.value) && (
          <div className="mt-3 rounded-md border border-white/10 bg-white/5 p-3 text-sm text-white/80">
            No Cardmarket price rows yet — wiring will mirror the Pokémon page.
          </div>
        )}
      </div>
    </section>
