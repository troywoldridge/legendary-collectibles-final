...
type SetRow = { id: string; name: string | null; series: string | null; release_date: string | null; logo_url: string | null; symbol_url: string | null; };
type ItemRow = { id: string; name: string | null; small_image: string | null; large_image: string | null; rarity?: string | null; };
type SearchParams = Record<string, string | string[] | undefined>;

const CATEGORY = { label: "Sports Cards", baseListHref: "/categories/sports/sets", bannerCfId: "f95ef753-c5fd-4079-9743-27cf651fd500" };
...
async function getSet(_id: string): Promise<SetRow | null> { return { id:_id, name:_id.replace(/-/g," "), series:null, release_date:null, logo_url:null, symbol_url:null }; }
async function getItems(_opts:{ setId:string; q:string|null; offset:number; limit:number; }):Promise<{rows:ItemRow[]; total:number;}> { return { rows: [], total: 0 }; }

export default async function SportsSetDetailPage({ params, searchParams }:{ params: Promise<{id:string}>; searchParams: Promise<SearchParams>; }) {
  ...
  const { rows, total } = await getItems({ setId:setParam, q, offset:(reqPage-1)*perPage, limit:perPage });
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-sm text-white/80">Brand / year browser</div>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-sm text-white/80">Showing {from}-{to} of {total} cards{q ? " (filtered)" : ""}</div>
        <div className="flex flex-wrap gap-3">
          ...
          <form action={baseHref} method="get" className="flex items-center gap-2">
            ...
            <input type="hidden" name="page" value="1" />
            <input name="q" defaultValue={q ?? ""} placeholder="Search (name/id)…" className="w-[240px] md:w-[320px] rounded-lg border border-white/20 bg-white/10 px-3 py-1.5 text-sm text-white placeholder:text-white/60 outline-none focus:ring-2 focus:ring-white/50" />
            ...
            {q && <Link href={buildHref(baseHref, { perPage, page: 1 })} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white hover:bg-white/15">Clear</Link>}
          </form>
        </div>
      </div>
      ...
      {/* Grid */}
      {rows.length === 0 ? (
        <div className="rounded-xl border border-white/15 bg-white/5 p-6 text-white/90 backdrop-blur-sm">No cards yet — data coming soon.</div>
      ) : (
